...
export const Admin = () => {
  ...
  const [selectedUser, setSelectedUserDetails] = useState<{
    name: string;
    email: string;
    phone: string;
    is_student: boolean;
  ...
  const handleSyncMenu = async () => {
    if (
      !window.confirm(
        "WARNING: This will DELETE all current products and categories and replace them with the new menu. Continue?",
      )
    )
      return;
    ...
    try {
      ...
      const productData = MOCK_PRODUCTS.map(({ id, ...p }) => ({
        ...p,
        is_available: true,
        rating: 4.5,
        reviews: 20,
      ...
      alert("Menu successfully replaced and synchronized!");
      window.location.reload();
    } catch (error: any) {
      console.error("Sync Error:", error);
      alert("Failed to sync menu: " + (error.message || "Unknown error"));
    } finally {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] text-center px-6 animate-in fade-in zoom-in duration-300">
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm max-w-xs leading-relaxed">
          This area is restricted to JK Restaurant staff only. Please log in
          with an admin account to proceed.
        </p>
        <button
          onClick={() => navigate("/")}
  ...
  const derivedCategories = useMemo(() => {
    ...
    products.forEach((p) => {
      ...
      if (!map.has(key)) {
        ...
        const catImg =
          products.find((prod) => (prod.sub_category || prod.name) === key)
            ?.image_url || "";
  ...
  const syncDerivedCategories = async () => {
    ...
    try {
      ...
      if (errorMsg) {
        alert(
          `Sync partially completed (${successCount} added). Error: ${errorMsg}`,
        );
  ...
  const filteredMenuProducts = useMemo(() => {
    return products.filter((p) => {
      ...
      const matchCat =
        menuCategoryFilter === "All" || pSub === menuCategoryFilter;
      const matchSearch =
      ...
      <div className="flex gap-3 mb-6 overflow-x-auto hide-scrollbar">
        <div className="bg-gradient-to-br from-brand-500 to-brand-600 rounded-xl p-4 min-w-[140px] text-white shadow-sm flex-1">
          ...
          <p className="text-[10px] mt-1 opacity-80">
            {adminOrders.filter((o) => o.status === "completed").length}{" "}
            completed sales
          </p>
        </div>
        <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl p-4 min-w-[140px] shadow-sm flex-1 transition-colors">
          ...
          <p className="text-[10px] mt-1 text-gray-400 dark:text-gray-500 font-medium">
            {adminOrders.length} total ever
          </p>
        </div>
      </div>
        ...
        <div className="space-y-4">
          {adminOrders.length === 0 ? (
            ...
            adminOrders.map((order) => (
              <div
                key={order.id}
                className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-5 shadow-sm transition-all hover:border-brand-200 dark:hover:border-brand-900/50"
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="font-black text-[15px] dark:text-white leading-none mb-1.5 uppercase tracking-tight">
                      {order.user_name}
                    </h3>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-black text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-800 px-2 py-0.5 rounded-md uppercase tracking-wider">
                  ...
                  <div
                    className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase border animate-pulse-slow ${getStatusColor(order.status)}`}
                  >
                    {order.status}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 mt-4 pt-4 border-t border-gray-50 dark:border-gray-800">
                  ...
                  {order.payment_screenshot_url && (
                    <button
                      onClick={() =>
                        setEnlargedScreenshot(order.payment_screenshot_url)
                      }
      ...
      {activeTab === "promos" && (
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-5 shadow-sm">
            <h3 className="font-black text-sm text-gray-800 dark:text-white mb-4 uppercase tracking-wider">
              Create Promo Code
            </h3>
          ...
          <div className="space-y-3 mt-6">
            <h3 className="font-black text-sm text-gray-800 dark:text-white uppercase tracking-wider mb-3">
            ...
            {promos.length === 0 ? (
              ...
              promos.map((promo) => (
                <div
                  key={promo.id}
                  className="flex items-center justify-between bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-4 rounded-xl shadow-sm"
                >
                  <div>
                    <h4 className="font-black text-brand-600 dark:text-brand-400 text-lg uppercase tracking-wider">
                      {promo.code}
                    </h4>
                    <p className="text-xs font-bold text-gray-500 dark:text-gray-400 mt-1">
                      {promo.discount_type === "percentage"
                        ? `${promo.discount_value}% OFF`
                        : `₹${promo.discount_value} FLAT RATE`}
                    </p>
                  </div>
                  <div className="flex items-center gap-4">
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        ...
                        onChange={(e) =>
                          togglePromo(promo.id, e.target.checked)
                        }
                      />
      ...
      {activeTab === "menu" && (
        <div className="space-y-4">
          ...
          {categories.length === 0 && derivedCategories.length > 0 && (
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-5 shadow-sm animate-in fade-in slide-in-from-top duration-500 mb-6">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h4 className="font-black text-[13px] text-amber-800 dark:text-amber-300 uppercase tracking-wider">
                    Sync Menu Categories
                  </h4>
                  <p className="text-[11px] text-amber-600 dark:text-amber-400 mt-0.5 leading-tight font-medium">
                    We found <strong>{derivedCategories.length}</strong>{" "}
                    categories in your menu. Save them to the DB to enable
                    images and editing.
                  </p>
                  <button
                  ...
                  >
                    <RefreshCw
                      size={12}
                      className={isSyncingCategories ? "animate-spin" : ""}
                    />
                    {isSyncingCategories
                      ? "Syncing..."
                      : `Sync ${derivedCategories.length} Items`}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
              <h2 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight leading-none mb-1">
                Menu Manager
              </h2>
              <p className="text-[10px] text-gray-500 dark:text-gray-400 font-black uppercase tracking-widest">
                {products.length} Items on Database
              </p>
            </div>
            <div className="flex gap-2 w-full sm:w-auto">
              <button
                ...
                className="flex-1 sm:flex-none px-4 py-2.5 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 font-black text-[10px] uppercase tracking-widest rounded-xl border border-indigo-100 dark:border-indigo-800 shadow-sm hover:bg-indigo-100 transition-all flex items-center justify-center gap-2"
              >
                <RefreshCw
                  size={14}
                  className={isSyncingMenu ? "animate-spin" : ""}
                />
                {isSyncingMenu ? "Syncing..." : "Sync New Menu"}
              </button>
              <button
                onClick={() => setShowAddModal(true)}
                className="flex-1 sm:flex-none px-4 py-2.5 bg-brand-500 text-white font-black text-[10px] uppercase tracking-widest rounded-xl shadow-xl shadow-brand-500/20 hover:bg-brand-600 active:scale-95 transition-all flex items-center justify-center gap-2"
              >
          ...
          {groupedMenuProducts.map(({ category, items }) => {
            ...
            const categoryImage =
              ...
              derivedCategories.find(
                (sc: any) => sc.name.toLowerCase() === category.toLowerCase(),
              )?.image;
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <h3 className="font-black text-sm text-gray-800 dark:text-white uppercase tracking-tight leading-none mb-0.5">
                          {category}
                        </h3>
                      ...
                      onClick={async () => {
                        if (categoryData) {
                          ...
                          if (!error && data) {
                            ...
                            alert(
                              "Could not initialize category for editing. Please try the Sync button at the top.",
                            );
                          }
                        }
                      }}
                ...
                {!collapsedCategories.has(category) && (
                  <div className="space-y-3 mb-4">
                    {items.map((product) => (
                      <div
                        key={product.id}
                        className={`bg-white dark:bg-gray-900 rounded-xl shadow-sm border p-4 transition-colors ${product.is_available ? "border-gray-100 dark:border-gray-800" : "border-red-200 dark:border-red-900/50 bg-red-50/30 dark:bg-red-900/5"}`}
                      >
                        <div className="flex gap-3 mb-3">
                          ...
                          <div className="flex-1">
                            <div className="flex justify-between items-start">
                              <div>
                                <h4
                                  className={`font-bold text-sm ${product.is_available ? "text-gray-800 dark:text-white" : "text-gray-500"}`}
                                >
                                  {product.name}
                                </h4>
                        ...
                        <div className="grid grid-cols-3 gap-2 mt-2 pt-2 border-t border-gray-100 dark:border-gray-800">
                          ...
                          <div>
                            <label className="text-[10px] text-brand-600 dark:text-brand-400 uppercase font-bold tracking-wider">
                              Student Price
                            </label>
                            <div className="flex items-center mt-1">
                              <span className="text-brand-600 dark:text-brand-400 text-sm font-bold mr-1">
                                ₹
                              </span>
                              <input
                                type="number"
                                ...
                                onChange={(e) =>
                                  updateProduct(product.id, {
                                    student_price: Number(e.target.value),
                                  })
                                }
                          ...
                          <div>
                            <label className="text-[10px] text-blue-600 dark:text-blue-400 uppercase font-bold tracking-wider">
                              Prep (m)
                            </label>
                            <div className="flex items-center mt-1">
                              <input
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="space-y-8">
            ...
            <section>
              <div className="flex items-center justify-between mb-4 px-1">
                <h3 className="font-black text-xs uppercase tracking-widest text-gray-400">
                  Student Community ({studentUsers.length})
                </h3>
              </div>
            ...
            <section>
              <div className="flex items-center justify-between mb-4 px-1">
                <h3 className="font-black text-xs uppercase tracking-widest text-gray-400">
                  Regular Customers ({regularUsers.length})
                </h3>
              </div>
              <div className="grid gap-3">
                {regularUsers.length > 0 ? (
                  ...
                  <p className="text-xs text-gray-400 italic px-1">
                    No regular customers found matching your search.
                  </p>
                )}
              </div>
            </section>
      ...
      {selectedUser && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative w-full max-w-md bg-white dark:bg-gray-950 rounded-t-[2.5rem] sm:rounded-[2.5rem] p-8 shadow-2xl animate-in slide-in-from-bottom duration-500 outline-none">
            <button
              onClick={() => setSelectedUserDetails(null)}
              className="absolute top-6 right-6 p-2 bg-gray-50 dark:bg-gray-900 rounded-full text-gray-400"
            >
              <X size={20} />
            </button>
            ...
            <div className="text-center mb-8">
              ...
              <div className="flex items-center justify-center gap-2 mt-2">
                <span
                  className={`text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-widest ${selectedUser.is_student ? "bg-indigo-50 dark:bg-indigo-900/30 text-indigo-500 border border-indigo-100 dark:border-indigo-800" : "bg-gray-50 dark:bg-gray-800 text-gray-400 border border-gray-100 dark:border-gray-700"}`}
                >
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-3xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in duration-300 border border-gray-100 dark:border-gray-800">
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-xs font-black text-gray-400 dark:text-gray-500 mb-2 uppercase tracking-widest">
                  Category Name
                </label>
                <input
                  type="text"
              ...
              {editingCategory.image_url && (
                <div className="flex items-center gap-4 bg-gray-50 dark:bg-gray-800/50 p-3 rounded-2xl border border-gray-100 dark:border-gray-800">
                  ...
                  <p className="text-[10px] font-bold text-gray-400 uppercase leading-snug">
                    Preview of how it appears on Home page
                  </p>
                </div>
              )}
                    ...
                    try {
                      ...
                      if (editingCategory.name !== originalCategoryName) {
                        const productsToUpdate = products.filter(
                          (p) =>
                            (p.sub_category || p.name) === originalCategoryName,
                        );
                        for (const product of productsToUpdate) {
                          await updateProduct(product.id, {
                            sub_category: editingCategory.name,
                          });
                        }
                        await fetchProducts();
                      }
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-md p-5 shadow-xl border border-gray-100 dark:border-gray-800 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  maxLength={500}
                  className="w-full border border-gray-300 dark:border-gray-700 rounded-lg p-2.5 text-sm dark:bg-gray-800 dark:text-white outline-none focus:border-brand-500 h-20"
                  placeholder="Delicious soft naan..."
                  onChange={(e) =>
                ...
                <div>
                  ...
                  {!isAddingNewCategory ? (
                    ...
                    >
                      ...
                      <option value="NEW" className="text-brand-500 font-bold">
                        ➕ Add New Group
                      </option>
                    </select>
                  ) : (
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-xs font-bold text-brand-600 dark:text-brand-400 mb-1">
                    Student Price
                  </label>
                  <input
                    type="number"
              ...
              <button
                onClick={() => {
                  if (newDish.name && newDish.base_price) {
                    const finalSubCategory = isAddingNewCategory
                      ? customCategory
                      : newDishSubCategory;
                    if (!finalSubCategory)
                      return alert(
                        "Please enter or select a menu group (Sub-Category)",
                      );
        ...
        >
          ...
          >
            <img
              ...
              className="max-w-full max-h-[85vh] rounded-2xl shadow-2xl animate-in zoom-in-95 duration-500 border border-white/10"
              alt="Payment Proof"
            />
